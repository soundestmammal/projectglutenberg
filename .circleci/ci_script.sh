...
aws elasticbeanstalk create-application-version --application-name "oasis" --version-label v$CIRCLE_BUILD_NUM --description="New Version number $CIRCLE_BUILD_NUM" --source-bundle S3Bucket="s3://$S3_BUCKET/oasis",S3Key="app_v_25.zip" --auto-create-application --region="us-east-2"

# deploying the new version to the given environment
aws elasticbeanstalk update-environment --application-name "oasis" --environment-name "Oasis-env" --version-label v25 --region="us-east-2"
