...
describe('Business Component', () => {
    const props = {
        ...
        location: {
            display_address: ['1847 Wantagh Ave', 'Wantagh, NY 11793'],
        },
        phone: '0123456789',
    };
    ...
    it('should render correct number of photos', () => {
        ...
        expect(wrapper.find('.individual-photo').length).toEqual(
    ...
    it('should render each restaurant photo in correct order 0,1,2,0', () => {
        ...
        expect(wrapper.find('.individual-photo').at(0).prop('src')).toEqual(
            props.photos[0]
        );
        expect(wrapper.find('.individual-photo').at(1).prop('src')).toEqual(
            props.photos[1]
        );
        expect(wrapper.find('.individual-photo').at(2).prop('src')).toEqual(
            props.photos[2]
        );
        expect(wrapper.find('.individual-photo').at(3).prop('src')).toEqual(
            props.photos[0]
        );
    });
    ...
    it('should show the correct title', () => {
        ...
        expect(wrapper.find('.business-title').render().text()).toEqual(
            props.name
        );
    });
    ...
    it('should render the correct price', () => {
        ...
        expect(wrapper.find('.business-price').render().text()).toEqual(
            props.price
        );
    });
    ...
    it('should show the properly formatted category string', () => {
        ...
        expect(wrapper.find('.business-categories').render().text()).toEqual(
            props.categories[0].title +
                ', ' +
                props.categories[1].title +
                ', ' +
                props.categories[2].title
        );
    });
    ...
    it('should render the correct open time', () => {
        ...
        expect(wrapper.find('.hours-time').at(0).render().text()).toEqual(
            '9:00 AM'
        );
    });
    ...
    it('should render the correct close time', () => {
        ...
        expect(wrapper.find('.hours-time').at(1).render().text()).toEqual(
            '5:00 PM'
        );
    });
    ...
    it("should render the text 'open now' if it is open now", () => {
        ...
        expect(wrapper.find('.open-now').at(0).render().text()).toEqual(
            'Open now!!!'
        );
    });
    ...
    it('should render the address', () => {
        ...
        expect(
            wrapper.find('.business-address').render().find('span').length
        ).toEqual(2);
    });
